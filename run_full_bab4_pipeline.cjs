...
async function runPipeline() {
    console.log('Loading BAB-4.pdf...');
    const pdfjsLib = await import('pdfjs-dist/legacy/build/pdf.mjs');
    const pdfPath = path.resolve('BAB-4.pdf');
    const data = new Uint8Array(fs.readFileSync(pdfPath));
    const loadingTask = pdfjsLib.getDocument({ data });
    const pdfDoc = await loadingTask.promise;

    console.log(`BAB-4 has ${pdfDoc.numPages} pages.`);

    const outputDir = path.resolve('extracted_pdf_images_bab4');
    ...
    fs.writeFileSync('FULL_MODULE_BAB_4.md', fullDoc.join('\n'), 'utf-8');
    console.log('Successfully created FULL_MODULE_BAB_4.md! Length:', fullDoc.join('\n').length);
}
