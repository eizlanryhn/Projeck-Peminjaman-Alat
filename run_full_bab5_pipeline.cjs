...
async function runPipeline() {
    console.log('Loading BAB-5.pdf...');
    const pdfjsLib = await import('pdfjs-dist/legacy/build/pdf.mjs');
    const pdfPath = path.resolve('BAB-5.pdf');
    const data = new Uint8Array(fs.readFileSync(pdfPath));
    const loadingTask = pdfjsLib.getDocument({ data });
    const pdfDoc = await loadingTask.promise;

    console.log(`BAB-5 has ${pdfDoc.numPages} pages.`);

    const outputDir = path.resolve('extracted_pdf_images_bab5');
    ...
    fs.writeFileSync('FULL_MODULE_BAB_5.md', fullDoc.join('\n'), 'utf-8');
    console.log('Successfully created FULL_MODULE_BAB_5.md! Length:', fullDoc.join('\n').length);
}
